...
| Silent degradation when API key is missing/placeholder | Medium | High | **Fixed** (audit pattern (d); sibling precedent: Execution `a5a9725`, Decision `f9d3230`+`1449b44`). `config.simulation_mode()` (`config/settings.py`) now treats empty, missing, *and* the `.env.example` sentinel (`your_openai_api_key_here`) as simulation mode — no truthiness-only check — and `config.simulation_reason()` logs one explicit line naming the mode and why on every run. Verified with a socket-connect guard (not timing): with the placeholder key set, `socket.socket.connect` is monkey-patched to raise if invoked, and `call_openai()` still returns the deterministic simulated response with zero connect calls. Separately, `pipeline/ai_processor.py` no longer folds a real 401 into the generic handler: `openai.AuthenticationError` is caught first and logged as an ERROR naming the cause before falling back; other `openai.OpenAIError` (timeouts, connection errors, rate limits) still fall back with one WARNING line, not silence (verified live: an unreachable-network run produced exactly one WARNING; a mocked 401 produced exactly one ERROR naming "API key rejected") | Misconfiguration *between* environments (e.g. a real key that expires mid-deployment, or a key valid in one account but not another) is still human-dependent — the fix closes the placeholder/missing-key blind spot, not every possible key-validity edge case |
...
| Validation thresholds go stale vs reality | Medium | Medium | `CONFIDENCE_THRESHOLD` centralized in `config/settings.py:36` (env var, default `0.60`), read in one place (`pipeline/router.py`). A dedicated outcome feedback loop exists in the sibling AI Decision Engine (see its ADR-0001), not enforced here, and outcome-based impact scoring exists in the sibling Impact engine — neither recalibrates this repo's threshold | No automated recalibration in this repo — a threshold set at launch stays fixed until someone manually revisits `config/settings.py`; staleness is invisible until outcomes drift |
